...
(async function () {
  ...
  const ICON_MATCH = {
    ...
    snapRadius: 6,
    snapStep: 1,
    // Chest auto-detect + scan (TEST MODE)
    chestTestEnabled: true,    // keep manual; this only logs events for testing
    chestCaptureSize: 680,     // capture size around mouse when chest text detected
    chestOcrSize: 560,         // smaller OCR window around mouse to detect the chest UI
    chestScanStep: 4,          // sliding-window step for scanning icons (lower = slower, more accurate)
    chestMaxHits: 12,          // max icon hits to report
    chestMinAccepted: 0.72,    // fallback acceptance if adaptive disabled
    chestCooldownMs: 3500,     // don't rescan repeatedly while chest stays open
...
// ------------------------------
// Barrows Chest auto-detect + icon scan (TEST MODE)
// Keeps manual submit. This only runs when chestTestEnabled is true.
// ------------------------------
let __lastChestScanAt = 0;

function __captureRegionAbs(x, y, w, h) {
  try {
    if (!(window.A1lib && typeof A1lib.capture === "function")) return null;
    const id = A1lib.capture(x | 0, y | 0, w | 0, h | 0);
    if (!id) return null;
    const capProps = __getImgProps(id);
    if (!capProps) return null;
    return { id, capProps, x: x | 0, y: y | 0, w: w | 0, h: h | 0 };
  } catch (e) {
    return null;
  }
}

function __regionHasKeywordsOCR(regionId, w, h, keywordsLower) {
  // Lightweight OCR scan: we only need presence/absence of a couple strings.
  // Uses bindReadStringEx when available; falls back to bindReadString.
  if (!window.alt1 || !alt1.permissionPixel) return false;

  const fonts = ["chat", "interface", "small", "big", "chatmono", "xpcounter"];
  const yStep = 6;
  const xStep = 32;

  for (let fi = 0; fi < fonts.length; fi++) {
    const font = fonts[fi];
    const args = JSON.stringify({ fontname: font, allowgap: true });
    let seen = "";
    for (let yy = 0; yy < h; yy += yStep) {
      for (let xx = 0; xx < w; xx += xStep) {
        let s = "";
        try { s = alt1.bindReadStringEx(regionId, xx, yy, args) || ""; }
        catch (e) { try { s = alt1.bindReadString(regionId, font, xx, yy) || ""; } catch (e2) { s = ""; } }
        if (!s) continue;
        seen += " " + String(s).toLowerCase();
        // cheap early checks
        let ok = true;
        for (let k = 0; k < keywordsLower.length; k++) {
          if (seen.indexOf(keywordsLower[k]) === -1) { ok = false; break; }
        }
        if (ok) return true;
      }
    }
...
function __detectBarrowsChestAroundMouse() {
  // Detects chest UI by OCR for both "BARROWS CHEST" and "TAKE ALL" in a region around the mouse.
  // Returns { ok, x,y,w,h } for a larger capture window if detected.
  if (!window.alt1 || !alt1.permissionPixel) return { ok: false, reason: "No pixel permission." };

  const pos = getMousePos();
  if (!pos) return { ok: false, reason: "Mouse position unavailable." };

  const ocrSize = (ICON_MATCH.chestOcrSize == null ? 560 : ICON_MATCH.chestOcrSize) | 0;
  const w = ocrSize, h = ocrSize;
  const half = (w >> 1);
  const x = Math.max(0, (pos.x | 0) - half) | 0;
  const y = Math.max(0, (pos.y | 0) - half) | 0;

  let rid;
  try { rid = alt1.bindRegion(x, y, w, h); }
  catch (e) { return { ok: false, reason: "bindRegion failed." }; }

  const ok = __regionHasKeywordsOCR(rid, w, h, ["barrows chest", "take all"]);
  return ok ? { ok: true, x, y, w, h } : { ok: false, reason: "keywords not found" };
}

function __scanForIconsInCapture(cap, templates) {
  if (!cap || !cap.capProps || !templates || !templates.length) return [];

  __buildTemplateFeatures(templates);

  const capProps = cap.capProps;
  const sSize = ICON_MATCH.sampleSize | 0;
  const step = (ICON_MATCH.chestScanStep == null ? 4 : ICON_MATCH.chestScanStep) | 0;
  const maxHits = (ICON_MATCH.chestMaxHits == null ? 12 : ICON_MATCH.chestMaxHits) | 0;

  const useEdges = ICON_MATCH.useEdges === true;
  const ew = (ICON_MATCH.edgeWeight === undefined ? 0.65 : ICON_MATCH.edgeWeight);

  // Limit scan to likely reward area: below title, above buttons.
  const y0 = Math.max(0, 42);
  const y1 = Math.max(y0, (capProps.height - 110) | 0);

  const poolsBySize = {};
  for (let i = 0; i < templates.length; i++) {
    const t = templates[i];
    if (!t) continue;
    const sz = (t.size | 0) || 32;
    if (!poolsBySize[sz]) poolsBySize[sz] = [];
    poolsBySize[sz].push(t);
  }

  const hits = [];

  function considerHit(hit) {
    // Non-max suppression: suppress hits that overlap heavily (regardless of name).
    const cx = hit.x + (hit.iconSz >> 1);
    const cy = hit.y + (hit.iconSz >> 1);
    const r = Math.max(12, (hit.iconSz >> 1));

    for (let i = 0; i < hits.length; i++) {
      const h = hits[i];
      const hx = h.x + (h.iconSz >> 1);
      const hy = h.y + (h.iconSz >> 1);
      const dx = cx - hx, dy = cy - hy;
      if ((dx*dx + dy*dy) <= (r*r)) {
        if (hit.score > h.score) hits[i] = hit;
        return;
      }
    }
    hits.push(hit);
  ...
  const sizesToTry = Object.keys(poolsBySize).map(x => x|0).sort((a,b)=>a-b);

  for (let si = 0; si < sizesToTry.length; si++) {
    const iconSz = sizesToTry[si];
    const pool = poolsBySize[iconSz];
    if (!pool || !pool.length) continue;
    if (capProps.width < iconSz || capProps.height < iconSz) continue;

    for (let y = y0; y <= (y1 - iconSz); y += step) {
      for (let x = 0; x <= (capProps.width - iconSz); x += step) {

        // Compute best + second for this window across templates of this size
        ...
        for (let i = 0; i < pool.length; i++) {
          ...
          if (!best || score > best.score) {
            ...
            second = { name: t.name, size: t.size, score };
          }
        }
        ...
        // Early stop if we already have plenty of strong hits
        if (hits.length >= maxHits * 2) {
          // keep scanning a bit but avoid runaway
        }
      }
    }
  }
...
async function __chestAutoTestTick() {
  try {
    if (!(ICON_MATCH && ICON_MATCH.chestTestEnabled === true)) return;
    if (!isSetupReady()) return;
    const now = Date.now();
    const cd = (ICON_MATCH.chestCooldownMs == null ? 3500 : ICON_MATCH.chestCooldownMs) | 0;
    if (now - __lastChestScanAt < cd) return;

    const det = __detectBarrowsChestAroundMouse();
    if (!det.ok) return;

    // Chest detected near mouse: capture bigger area around mouse for scanning.
    const capSize = (ICON_MATCH.chestCaptureSize == null ? 680 : ICON_MATCH.chestCaptureSize) | 0;
    const pos = getMousePos();
    if (!pos) return;
    const half = capSize >> 1;
    const rx = Math.max(0, (pos.x | 0) - half) | 0;
    const ry = Math.max(0, (pos.y | 0) - half) | 0;

    const cap = __captureRegionAbs(rx, ry, capSize, capSize);
    if (!cap) return;

    const templates = await ensureIconTemplatesLoaded();
    if (!templates || !templates.length) return;

    const hits = __scanForIconsInCapture({ capProps: cap.capProps }, templates);

    __lastChestScanAt = now;

    if (DEBUG_ICON_MATCH) {
      try {
        console.group("[BARROWS CHEST TEST] hits");
        console.table(hits.map((h, idx) => ({
          rank: idx + 1,
          name: h.name,
          score: Number((h.score || 0).toFixed(4)),
          iconSz: h.iconSz,
          gap: Number((h.gap || 0).toFixed(4)),
          ratio: (h.ratio === Infinity ? "∞" : Number((h.ratio || 0).toFixed(4))),
          x: h.x, y: h.y
        })));
        console.groupEnd();
      } catch (e) {}
    ...
    if (!hits.length) {
      showEvent("Chest test", "Barrows chest detected — no icon hits yet.", "warn", true, true);
      return;
    }

    const topNames = hits.slice(0, 4).map(h => h.name).join(", ");
    showEvent("Chest test", `Detected ${hits.length} icon(s): ${topNames}${hits.length > 4 ? "…" : ""}`, "ok", true, true);
  } catch (e) {
    // keep silent in poll
  }
}
...
// ---------- chat reader ----------
  ...
  async function poll() {
    ...
    // Barrows Chest auto-test (runs independently of chat)
    __chestAutoTestTick();
