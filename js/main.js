...
(function () {
  ...
  function setSetupSelectionEnabled(enabled) {
    // Bingo/Team custom selects are button-driven; disable their buttons.
    if (ui.bingoBtn) ui.bingoBtn.disabled = !enabled;
    if (ui.teamBtn) ui.teamBtn.disabled = !enabled;
    if (ui.btnLockSetup) ui.btnLockSetup.disabled = !enabled || ui.btnLockSetup.disabled; // keep existing validation too
    if (ui.setupHint) {
      ui.setupHint.textContent = enabled
        ? "Step 2: choose Bingo + Team, then lock them. Chat auto-detect will lock chat automatically if enabled."
        : "Step 1: enter your IGN and lock it first.";
    }
  }
...
function renderIgnLockedUI(locked) {
  ...
  window.addEventListener("storage", (e) => {
    ...
    if (e.key === LS.chatPos) {
      playBeep(e.newValue ? "ok" : "warn");
      // If chat calibration/lock changed in another window, sync runtime state so the
      // premium status line clears stale messages (e.g. "Chat moved...").
      try { onExternalChatPosChange(e.newValue); } catch (err) {}
    }
  });
  ...
  function updateLockButtonEnabled() {
    ...
    const il = (localStorage.getItem(LS.ignLocked) || "") === "1";
    ui.btnLockSetup.disabled = !(il && b > 0 && t > 0);
  }
  ...
  // Sync chat calibration/lock state when localStorage changes in another window (e.g. Settings popup).
  // Without this, the premium status line can get "stuck" on an older message until a full reload.
  function onExternalChatPosChange(newValue) {
    // storage events don't fire in the same window that wrote the value, only other windows/tabs.
    // This is safe to run even if chat isn't initialised yet.
    try {
      if (!isAlt1) return;

      const hasPos = !!newValue;

      // Ensure the reader exists if we just got a calibration.
      if (hasPos && !chatReader) {
        const ok = initChatReader();
        if (!ok) return;
      }

      if (!hasPos) {
        // Unlocked/cleared elsewhere
        ...
        setChatPillMissing();
        // Clear any stale premium status line immediately.
        showEvent("Chat unlocked.", "Status", "warn", true, false);
        return;
      }

      // Locked/calibrated elsewhere
      let pos = null;
      try { pos = JSON.parse(newValue); } catch (e) { pos = null; }
      if (!pos) return;

      if (chatReader) chatReader.pos = pos;
      if (typeof chatState !== "undefined" && chatState) {
      ...
      // Overwrite any stale status line (e.g. "Chat moved...") right away.
      showEvent("Chatbox calibrated & locked ✅", "Status", "ok", true, false);

      // If already running, also reflect the normal runtime status without spamming the feed.
...
function initChatReader() {
  ...
  function tryFindChatbox(tag) {
    ...
    try {
      ...
      if (chatReader.pos !== null) {
        ...
        // If we don't have a stored chat position yet, auto-lock it immediately.
        // This prevents the UI from getting stuck in a "moved" state on first start.
        if (!localStorage.getItem(LS.chatPos)) {
          try {
            ...
            chatState.confPct = 95;
            setChatPillLocked();
            addFeed(`Chatbox auto-detected & locked ✅ (${tag}).`, "ok");
            tryOverlayRect(chatReader.pos, false);
            // Clear any stale premium status line.
            showEvent("Running. Waiting for drops…", "Status", "ok", false, false);
            return true;
          } catch (e) {
            // fall back to "found but not locked"
          }
        }
  ...
  // Ready to run/poll: allow missing chatPos if auto-detect is enabled
  function isRunReady() {
    const cl = !!localStorage.getItem(LS.chatPos);
    if (cl) return isConfigReady();
    return isConfigReady() && isAlt1 && loadSettings().autoDetect === true;
  }

  // Fully ready: chatPos is locked/set
  function isFullyReady() {
    const cl = !!localStorage.getItem(LS.chatPos);
    return isConfigReady() && cl;
  }
  ...
  function start() {
    ...
    if (!isRunReady()) {
      addFeed("Finish setup first: lock IGN, lock Bingo/Team, then lock chat (or enable Auto-detect chat).", "bad");
      return;
    }
    if (running) return;
  ...
  if (chatState.consecutiveEmpty >= 12 && loadSettings().autoDetect && !chatState.locked) {
    ...
    if (ok) {
      chatState.consecutiveEmpty = 0;
      // We found a new chatbox position, but it isn't locked (calibrated) yet.
      // Warn in the premium status line without overwriting the "auto-detected" feed entry.
      showEvent("Chat moved. Lock it again from Settings.", "Status", "warn", true, false);
    }
  }
  return;
}
